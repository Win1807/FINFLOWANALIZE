...
sap.ui.define([
 ...
 return BaseController.extend("fin.cash.flow.analyzer.controller.NewHierarchyController", {
  ...
  getCurrentAppState: function () {
   ...
   oSelectionVariant.removeParameter("DateIndicator");
   oSelectionVariant.addSelectOption("DateIndicator", "I", "EQ", this.getView().byId("idDateIndicator").getSelectedKey());
   ...
   oParseNavigationPromise.done(function (oAppData, oURLParameters, sNavType) {
    ...
    case sap.ui.generic.app.navigation.service.NavType.iAppState:
     ...
     if (that.util.isNull(dateIndicator)) {
      ...
      that.oCurrSmartFilterBar.getControlByKey("DateIndicator").setSelectedKey("1");
     } else {
      that.oCurrSmartFilterBar.getControlByKey("DateIndicator").setSelectedKey(dateIndicator[0].Low);
     }
    ...
    default:
     ...
     if (that.sHierType !== "CPS") {
      ...
      for (i = 0; i < prmtrList.length; i++) {
       ...
       if (prmtrList[i].PropertyName === "DateIndicator") {
        if (that.sHierType === "BKH" || that.sHierType === "LQH") {
         that.getView().byId("idDateIndicator").setSelectedKey(prmtrList[i].PropertyValue);
        }
       }
      }
  ...
  setRequestFilters: function(aFilters){
   ...
   //Set date indicator filter
   var dateIndicator = this.getView().byId("idDateIndicator").getSelectedKey();
   if (this.util.isNull(dateIndicator)) {
    aFilters.push(new sap.ui.model.Filter("DateIndicator", "EQ", "1"));
   } else {
    var oFilterDateInd = new sap.ui.model.Filter("DateIndicator", 'EQ', dateIndicator);
    aFilters.push(oFilterDateInd);
   }
  },
  ...
  handleMyTableDataRefreshReady: function (oData) {
   ...
   try {
    ...
    if (this.sHierType === "CPS") {
     ...
     this.aTableCols.push(new sap.ui.table.Column({
      width: "150px",
      label: this.oResourceBundle.getText("Currency"),
      template: oLabel,
      hAlign: "Left"
     }));
     oLabel = new sap.m.Label({
   ...
   for (var i = 0; i < oDataOrg["results"].length; i++) {
    ...
    if (this.sHierType === "CPS") {
     oNode = {
      ...
      currency: oDataOrg["results"][i].Currency,
      displayCurrency: oDataOrg["results"][i].DisplayCurrency,
      checked: true,
      level: sLevel,
  ...
  onHandleAfterVariantLoad: function (oEvent) {
   ...
   this.oCurrSmartFilterBar.getControlByKey("DateIndicator").setSelectedKey(oCustomFieldData.DateIndicator);
  ...
  onHandleBeforeVariantSave: function (oEvent) {
   ...
   var oDateIndicator = this.getView().byId("idDateIndicator").getSelectedKey();
