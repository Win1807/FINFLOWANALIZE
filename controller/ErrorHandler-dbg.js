...
sap.ui.define([
	...
	return UI5Object.extend("fin.cash.flow.analyzer.controller.ErrorHandler", {

		initODateErrorHandler: function(oController) {

			this._oComponent = oController.getComponent();
			this._oDataModel = oController.getView().getModel();
			this._oResourceBundle = oController.getView().getModel("i18n").getResourceBundle();
			...
			this._oDataModel.attachEvent("requestFailed", function(oEvent) {
				...
				if (jQuery.isEmptyObject(oParameters.response)) {
					...
					if (oParameters.response.statusCode !== 404 ||
						...
						oResponseText = jQuery.parseJSON(oEvent.getParameter("responseText"));
						if (oResponseText &&
							oResponseText.error &&
			...
			this._oDataModel.attachEvent("requestCompleted", function(oEvent) {
				...
				var sMessage, oResponseText;
				//temporary code
				// var oSmartable = this._oController.getView().byId("fin.cash.paymentdetail.display.TableDetailSmart");
				// oSmartable.getTable().setEnableBusyIndicator(false);
				...
				if (oParameters.success === true) {
					...
					if (oParameters.errorobject) {
						oResponseText = jQuery.parseJSON(oParameters.errorobject.responseText);
						if (oResponseText &&
							oResponseText.error &&
							oResponseText.error.innererror &&
							oResponseText.error.innererror.errordetails &&
							oResponseText.error.innererror.errordetails[0] &&
							oResponseText.error.innererror.errordetails[0].message) {
							sMessage = "(" + oParameters.errorobject.message + " " + oParameters.errorobject.statusCode + " " + oParameters.errorobject.statusText +
								")\r\n" + oResponseText.error.innererror.errordetails[0].message;
						}
